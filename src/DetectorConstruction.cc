...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
  ...
  G4double Boron_fraction= .01;
  G4double BoronWater_density = 1.0;
  G4Material* Boron = nist->FindOrBuildMaterial("G4_B");
  G4Material* Water = nist->FindOrBuildMaterial("G4_WATER");
  G4Material* BoronWater = new G4Material("BoronWater", BoronWater_density, 2);
  BoronWater->AddMaterial(Water, 90*perCent);
  BoronWater->AddMaterial(Boron, 10*perCent);
  ...
  G4double  length_Target = 800*mm;//800*mm
  ...
  G4LogicalVolume* logicalTarget =
    new G4LogicalVolume(solidTarget,          //its solid
                        ...
                        //BoronWater,           //its material
                        "Target");            //its name

  G4VPhysicalVolume* physicalTarget =
    new G4PVPlacement(0,                     //no rotation
                      G4ThreeVector(0, 0, 400*mm),       //at (0,0,400*mm)
